...
const Auction = artifacts.require('CyberSpawnNFTAuction');
const AccessControl = artifacts.require('CyberSpawnAccessControls');
const MockNft = artifacts.require('CyberSpawnNFT');

contract('cyber spawn auction', async ([alice, bob, dev, admin, minter]) => {
  ...
  beforeEach(async () => {
    ...
    //  Deploy contracts    //
    //////////////////////////
    this.accessControlContract = await AccessControl.new({
      from: minter
    })
  ...
  it('general flow', async () => {
    ...
    await this.auctionContract.resultAuction(tokenIdOfAlice, {
      from: alice
    })
  ...
  it("withdraw bid", async () => {
    ...
    await this.auctionContract.createAuction(tokenIdOfAlice, web3.utils.toWei('0.3'), now(), now() + 24 * 3600, {
      from: alice
    })
    ...
    await this.auctionContract.withdrawBid(tokenIdOfAlice, {
      from: bob
    })
